...
/**
 * NOTE: Guest users receive temporary authentication tokens when they make a booking.
 * These tokens are used to authenticate guest email requests and other guest-specific operations.
 * The temporary token is stored in localStorage and sent with the Authorization header.
 */
...
/**
 * Send booking confirmation email endpoint for guest users (uses temporary authentication)
 */
router.post('/send-guest-booking-confirmation', authenticateToken, async (req, res) => {
  try {
    ...
    // Validate that this is a guest booking (no user ID in the booking)
    if (booking.user_id) {
      return res.status(400).json({ 
        success: false, 
        message: 'This endpoint is for guest bookings only' 
      });
    }
    ...
    console.error('Error sending guest booking confirmation:', error);
    res.status(500).json({ 
      success: false, 
      message: 'Failed to send guest booking confirmation email', 
      error: error.message 
    });
  }
});
...
/**
 * Test email endpoint (for development/testing) - Uses temporary authentication for guest emails
 */
...
/**
 * Test SendGrid connection and configuration (no authentication required for connection testing)
 */
